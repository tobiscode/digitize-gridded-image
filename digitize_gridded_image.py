...
def project_point_on_line_3d(line: np.ndarray,
    ...
    for i in range(t.size):
        ...
        that = (np.dot(r_minus_q[i, :], q_minus_p[i, :])
                / np.dot(r_minus_q[i, :], r_minus_q[i, :]))
...
if __name__ == "__main__":
    ...
    prs.add_argument("filename", type=str,
                     help="Filename of image.")
    prs.add_argument("data_box", type=str,
                     help=("JSON-formatted string of (inclusive) pixel boundary coordinates "
                           "of the data in the order [left, top, right, bottom]."))
    prs.add_argument("legend_box", type=str,
                     help=("JSON-formatted string of (inclusive) pixel boundary coordinates "
                           "of the colorbar in the order [left, top, right, bottom]; "
                           "note that either left=right or top=bottom."))
    prs.add_argument("markers_pixel", type=str,
                     help=("JSON-formatted string of ascending pixel coordinates of the "
                           "tick labels (x coordinates if colorbar horzontal, y otherwise)."))
    prs.add_argument("markers_data", type=str,
                     help="JSON-formatted string of data values of the tick labels.")
    prs.add_argument("nx", type=int,
                     help="Number of data grid points in the x direction.")
    prs.add_argument("ny", type=int,
                     help="Number of data grid points in the y direction (defaults to `nx`).")
    prs.add_argument("output_name", type=str,
                     help="Output filename for data with either `.npy` or `.txt` extension.")
    prs.add_argument("--recreate-image", type=str, default=None,
                     help=("If provided, also save a debugging image with the extracted colors "
                           "to the specified filename."))
    prs.add_argument("--ignore-black", type=int, default=None,
                     help=("When extracting the dominant colors, ignore pixels this far away in "
    ...
    assert np.logical_xor(legend_box[0] == legend_box[2],
                          legend_box[1] == legend_box[3])
    ...
    borders_x = np.linspace(data_box[0], data_box[2] + 1, num=nx + 1).round().astype(int)
    borders_y = np.linspace(data_box[1], data_box[3] + 1, num=ny + 1).round().astype(int)
    ...
    for i in range(cmed.shape[0]):
        for j in range(cmed.shape[1]):
            # extract sub-image
            subimg = img[borders_y[i]:borders_y[i + 1],
                         borders_x[j]:borders_x[j + 1], :].reshape(-1, 3)
            # if ignoring black colors, remove the pixels that are too close
            # given the input radius
            if ignore_black is not None:
                not_black_ix = (np.sqrt(np.sum(subimg.astype(float)**2, axis=1))
                                > ignore_black)
